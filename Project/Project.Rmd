...
for(i in 1:length(vect)){
  ...
  for(j in 1:length(transform)){
    ...
    mtext(paste0("Shapiro: p-value = ", signif(shapiro.test(transform[[j]])$p.value, 
                                               digits=3)), side=1, line=-7, adj=0, cex=.8)
    mtext(paste0("ad.test: p-value = ", signif(ad.test(
      (transform[[j]]$residuals)^2, lag=sqrt(length(transform[[j]])), type=c("Ljung-Box"), fitdf = 11)$p.value, 
      digits=3), "_________________________________kpss.test: p-value = ", 
      signif(kpss.test(transform[[j]]$residuals)$p.value, digits=3)), side=1, line=-6, adj=0, cex=.8)
    model=ets(transform[[j]])
    print(summary(model))
    require(TSA)
    periodogram(transform[[j]], log = 'yes', xlim=c(0,0.025), title(main=transform_name[j]))
    abline(h=0)
    hist(transform[[j]], col="light blue", xlab="", main=transform_name[j], prob=TRUE)
    m <- mean(transform[[j]])
    std <- sqrt(var(transform[[j]]))
    curve( dnorm(x,m,std), add=TRUE)
    qqnorm(transform[[j]],main= paste0("Normal Q-Q Plot for Model ", transform_name[j]))
...
mtext(paste0("McLeod-Li: p-value = ", signif(Box.test((`SARIMA(4,1,4)x(2,0,1)_7`$residuals)^2, lag=sqrt(1228), type=c("Ljung-Box"), fitdf = 11)$p.value, digits=3), "_________________________________Shapiro: p-value = ", signif(shapiro.test(vect2[[j]]$residuals)$p.value, digits=3)),  side=1, line=-7, adj=0, cex=.8)


mxtext(pate0("Shapiro: p-value = ", signif(shapiro.test(vect2[[j]]$residuals)$p.value, digits=3)),  side=1, line=-7, adj=0, cex=.8)
mtext(paste0("ad.test: p-value = ", signif(ad.test((vect2[[j]]$residuals)^2, lag=sqrt(length(vect2[[j]])), type=c("Ljung-Box"), fitdf = 11)$p.value, digits=3), "_________________________________kpss.test: p-value = ", signif(kpss.test(vect2[[j]]$residuals)$p.value, digits=3)), side=1, line=-6, adj=0, cex=.8)

  
  plot.ts(get(paste0(vect1[i], ".bc")), main=paste0(vect1[i], ".bc"))
  abline(h=mean(get(paste0(vect1[i], ".bc"))), col= "blue")
  abline(lm(get(paste0(vect1[i], ".bc")) ~ as.numeric(1:length(get(paste0(vect1[i], ".bc"))))), col="red")
  x=acf2(get(paste0(vect1[i], ".bc")), main=paste0(vect1[i], ".bc"))
  model1=ets(get(paste0(vect1[i], ".bc")))
  print(summary(model1))
  print(ad.test(get(paste0(vect1[i], ".bc")))$p.value > 0.05)
  print(shapiro.test(get(paste0(vect1[i], ".bc")))$p.value > 0.05)

  plot.ts(get(paste0(vect1[i], ".log")), main=paste0(vect1[i], ".log"))
  abline(h=mean(get(paste0(vect1[i], ".log"))), col= "blue")
  abline(lm(get(paste0(vect1[i], ".log")) ~ as.numeric(1:length(get(paste0(vect1[i], ".log"))))), col="red")
  x=acf2(get(paste0(vect1[i], ".log")), main=paste0(vect1[i], ".log"))
  model=ets(get(paste0(vect1[i], ".log")))
  print(summary(model))
  print(ad.test(get(paste0(vect1[i], ".log")))$p.value > 0.05)
  print(shapiro.test(get(paste0(vect1[i], ".log")))$p.value > 0.05)

  plot.ts(get(paste0(vect1[i], ".sqrt")), main=paste0(vect1[i], ".sqrt"))
  abline(h=mean(get(paste0(vect1[i], ".sqrt"))), col= "blue")
  abline(lm(get(paste0(vect1[i], ".sqrt")) ~ as.numeric(1:length(get(paste0(vect1[i], ".sqrt"))))), col="red")
  x=acf2(get(paste0(vect1[i], ".sqrt")), main=paste0(vect1[i], ".sqrt"))
  model=ets(get(paste0(vect1[i], ".sqrt")))
  print(summary(model))
  print(ad.test(get(paste0(vect1[i], ".sqrt")))$p.value > 0.05)
  print(shapiro.test(get(paste0(vect1[i], ".sqrt")))$p.value > 0.05)

  hist(vect[[i]], col="light blue", xlab="", main=vect1[i])
  hist(get(paste0(vect1[i], ".log")), col="light blue", xlab="", main=paste0(vect1[i], ".log"))
  hist(get(paste0(vect1[i], ".bc")), col="light blue", xlab="", main=paste0(vect1[i], ".bc"))
  hist(get(paste0(vect1[i], ".sqrt")), col="light blue", xlab="", main=paste0(vect1[i], ".sqrt"))}
```
