...
Mortality1 = ts(covid$Mortality[10:336])
Mortality2 = ts(covid$Mortality[337:1238])
...
ad.test(Mortality)
require(TSA)
periodogram(Mortality, log = 'yes', xlim=c(0,0.025))
abline(h=0)
...
x=acf2(Mortality.bc)
model1=ets(Mortality.bc)
summary(model1)
ad.test(Mortality.bc)
...
model=ets(Mortality.log)
summary(model)
ad.test(Mortality.log)
...
acf2(`SARIMA(5,1,4)x(0,0,1)_7(fixed)`$residuals)
...
x=Box.test(`SARIMA(4,1,4)x(2,0,1)_7`$residuals, lag=7, type=c("Box-Pierce"), fitdf = 11)
x$p.value
