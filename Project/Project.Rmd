  ...
  for(j in 1:length(transform)){
    ...
    assign(paste0("diff_name_", transform_name[j]), diff_name)}
      ...
      if(list[k]==0){
        ...
        Bpx_Pierce_test <- append(Box_Pierce_test, Box.test(difference[[k]], lag = sqrt(length(difference[[k]])), type = c("Box-Pierce"), fitdf = 0))
      ...
      if(list[k]==1.12){
        ...
        assign(paste0(transform_name[j], "_Box-Pierce.test"), Bpx_Pierce_test)
...
options(scipen = 9)
# To choose parameter λ of the Box-Cox transformation for datset Mortality:
# Models = Dict$new("Mortality1" = Dict$new(), "Mortality2" = Dict$new(), "Mortality3" = Dict$new(), "Vaccinated1" = Dict$new(), "Death1" = Dict$new(), "Death2" = Dict$new(), "Death3" = Dict$new(), "Cases1" = Dict$new(), "Cases2" = Dict$new(), "Cases3" = Dict$new())
`_` = list()
bc = list()
log = list()
sqrt = list()
