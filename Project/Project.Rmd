...
Vaccinated3= data.frame(Vaccinated3)
...
```{r}
options(scipen = 9)
# To choose parameter λ of the Box-Cox transsignifion for each dataset:
...
for(i in 1:length(vect)){
  ...
  # Perform transsignifions, plot transformed data, histograms:
...
```{r, warning = FALSE}
options(scipen = 9)
# To choose parameter λ of the Box-Cox transsignifion for datset Mortality:
for(i in 1:length(vect)){
  bcTransform <- boxcox(vect[[i]]~ as.numeric(1:length(vect[[i]])))       # plot the graph
...
for(i in 1:length(vect)){
  ...
  for(j in 1:length(transform)){
    ...
    assign(paste0("diff_name_", transform_name[j]), diff_name)}
      ...
      if(list[k]==0){
        ad_test <- append(ad_test, ad.test(difference[[k]]))
        Data[x, 5] = signif(ad.test(difference[[k]])$p.value, 5)
        shapiro_test <- append(shapiro_test, shapiro.test(difference[[k]]))
        Data[x, 6] = signif(shapiro.test(difference[[k]])$p.value, 5)
        adf_test <- append(adf_test, adf.test(difference[[k]]))
        Data[x, 7] = signif(adf.test(difference[[k]])$p.value, 5)
        kpss_test <- append(kpss_test, kpss.test(difference[[k]]))
        Data[x, 8] = signif(kpss.test(difference[[k]])$p.value, 5)
        Box_Pierce_test <- append(Box_Pierce_test, Box.test(difference[[k]], lag = sqrt(length(difference[[k]])), type = c("Box-Pierce"), fitdf = 0))
        Data[x, 9] = signif(Box.test(difference[[k]], lag=sqrt(length(difference[[k]])), type=c("Box-Pierce"), fitdf = 0)$p.value, 5)
        Ljung_Box_test <- append(Ljung_Box_test, Box.test(difference[[k]], lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0))
        Data[x, 10] = signif(Box.test(difference[[k]], lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0)$p.value, 5)
        McLeod_Li_test <- append(McLeod_Li_test, Box.test((difference[[k]])^2, lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0))
        Data[x, 11] = signif(Box.test((difference[[k]])^2, lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0)$p.value, 5)
...
# To choose parameter λ of the Box-Cox transsignifion for datsets:
`_` = list()
bc = list()
log = list()
sqrt = list()
...
# To choose parameter λ of the Box-Cox transformation for datasets:
`_` = list()
bc = list()
log = list()
sqrt = list()
