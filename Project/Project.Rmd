...
Mortality.stat <- diff(Mortality, lag=1)
fit <- lm(Mortality.stat ~ as.numeric(1:length(Mortality.stat)))
plot.ts(Mortality.stat, main="(Mortality) differenced at lag 1");
abline(fit, col="red") 
abline(h=mean(Mortality.stat), col="blue")
mean(Mortality.stat)
var(Mortality.stat)
...
Mortality_1.7 <- diff(Mortality.stat, lag=7)
...
Mortality_1.12 <- diff(Mortality.stat, lag=12)
...
Mortality_1.14 <- diff(Mortality.stat, lag=14)
...
# Models for Model.stat

# Create the loop.vector (all the columns)
loop.vector <- 1:14

for (i in loop.vector) { # Loop over loop.vector
  assign(paste0("ARIMA_0_1_", i), arima(Mortality, order=c(0,1,i), method="ML"))
  assign(paste0("Residuals_ARIMA_0_1_", i), arima(Mortality, order=c(0,1,i), method="ML")$residuals)
  variable1 = paste0("ARIMA_0_1_", i)
  variable2 = paste0("Residuals_ARIMA0_1_", i)
  AICc(variable1)
}
...
for(i in 1:3){
  arima(Mortality.stat, order=c(i,1,1), seasonal = list(order = c(0,1,1), period = 12), method="ML")
  acf2(arima(Mortality.stat, order=c(i,1,1), seasonal = list(order = c(0,1,1), period = 12), method="ML"))
  AICc(arima(Mortality.stat, order=c(i,1,1), seasonal = list(order = c(0,1,1), period = 12), method="ML"))
}
