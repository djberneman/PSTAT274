...
for(i in 1:length(vect)){
  ...
  for(j in 1:length(transform)){
    ...
    transform[[j]]
    x=acf2(transform[[j]], title(main=transform_name[j]))
    mtext(paste0("Shapiro: p-value = ", signif(shapiro.test(transform[[j]])$p.value, digits=3)), side=1, line=-7, adj=0, cex=.8)
    mtext(paste0("ad.test: p-value = ", signif(ad.test(transform[[j]])$p.value, digits=3),"_________________________________kpss.test: p-value = ", signif(kpss.test(transform[[j]])$p.value, digits=3)), side=1, line=-6, adj=0, cex=.8)
    model=ets(transform[[j]])
    print(summary(model))
    require(TSA)
    nf <- layout( matrix(c(1,2, 3,4), ncol=2))
    periodogram(transform[[j]], log = 'yes', xlim=c(0,0.025), main=paste0(transform_name[j], " Periodogram"))
    abline(h=0)
