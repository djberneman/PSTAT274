...
for(i in 1:length(Data$Name)){
  ...
  for(x in c(6,7, 9:12)){
    if(Data[i,x]>=0.05){
      Data$Pass[i] = 1 + Data$Pass[i]}}
  if(Data[i, 8] <= 0.05){
    Data$Pass[i] = 1 + Data$Pass[i]}
  Variance_Order = data.frame(matrix(ncol=3, nrow=5))
  colnames(Variance_Order) = c("Levels", "Variance", "Rank")
  Variance_Order$Levels = c("Original", "Transformation", "Differencing", "Seasonality", "Current")
  Variance_Order$Variance[1:5] = Data[i, c(13:17)]
  ...
  Data$Variance_test[i] = paste0(Variance_Order$Rank[1], ", ", Variance_Order$Rank[2], ", ", Variance_Order$Rank[3], ", ", Variance_Order$Rank[4], ", ", Variance_Order$Rank[5])
  Fails = 0
  if(Data[i, 13] < Data[i, 14]){
    Fails=Fails+1}
  if((Data[i, 14] < Data[i, 15]) & (Data[i, 14] < Data[i, 16])){
    Fails=Fails+1}
  if((Data[i, 13] < Data[i, 15]) & (Data[i, 13] < Data[i, 16])){
    Fails=Fails+1}
  if((Data[i, 15] < Data[i, 17]) & (Data[i, 16] < Data[i, 17])){
    Fails=Fails+1}
  if(Data[i, 13] < Data[i, 17]){
    Fails=Fails+1}
  if(Data[i, 14] < Data[i, 17]){
    Fails=Fails+1}
  Data$Pass[i] = (6-Fails)/6 + Data$Pass[i]}
...
```{r}
Name = vector()
for(i in 1:length(Tier1$Name)){
  Name = append(Name, Tier$Name[i])}
...
Name = rbind(data.frame(Tier1$Name), data.frame(Tier2$Name), data.frame(Tier3$Name))
Tier2$Name
Tier3$Name
Tier1$Name
