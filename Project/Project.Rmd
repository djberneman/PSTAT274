...
for(i in 1:length(vect)){
  nt=length(vect[[i]])
  fit <- lm(vect[[i]] ~ as.numeric(1:nt))
  plot.ts(vect[[i]])
  abline(h=mean(vect[[i]]), col= "blue")
  abline(fit, col="red")
  title(main=vect1[i])
  hist(vect[[i]], col="light blue", xlab="", main=vect1[i])
  x=acf2(vect[[i]], main=vect1[i])
  print(shapiro.test(vect[[i]])$p.value > 0.05)
  var(vect[[i]])}
```
\
# Transformed Dataset
...
for(i in 1:length(vect)){
  ...
  lambda=bcTransform$x[which(bcTransform$y == max(bcTransform$y))]
  assign(paste0(vect1[i], ".bc"),  (1/lambda)*(vect[[i]]^lambda-1))
  assign(paste0(vect1[i], ".log"),  log(vect[[i]]))
  assign(paste0(vect1[i], ".sqrt"), sqrt(vect[[i]]))

  plot.ts(vect[[i]], main=vect1[i])
  abline(h=mean(vect[[i]]), col= "blue")
  abline(lm(vect[[i]] ~ as.numeric(1:length(vect[[i]]))), col="red")
  x=acf2(vect[[i]])
  model=ets(vect[[i]])
  print(summary(model))
  print(ad.test(vect[[i]])$p.value > 0.05)
  require(TSA)
  periodogram(vect[[i]], log = 'yes', xlim=c(0,0.025), title(main=vect1[i]))
  abline(h=0)
  print(shapiro.test(vect[[i]])$p.value > 0.05)
