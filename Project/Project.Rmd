...
for(i in 1:length(vect)){
  ...
  for(j in 1:length(transform)){
    ...
    # data1[i,1] = signif(ad.test(difference[[k]])$p.value, digits=3)
    # data1[i,2] = signif(shapiro.test(difference[[k]])$p.value, digits=3)
    # data1[i,3] = signif(kpss.test(difference[[k]])$p.value, digits=3)
    # data1[i,4] = signif(Box.test(difference[[k]], lag = sqrt(length(difference[[k]])), type = c("Box-Pierce"), fitdf = 0)$p.value, digits=3)
    # data1[i,5] = signif(Box.test(difference[[k]], lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3)
    # data1[i,6] = signif(Box.test((difference[[k]])^2, lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3)
    # i=i+1
    # require(TSA)
    # # nf <- par(mfrow=c(2,2))
