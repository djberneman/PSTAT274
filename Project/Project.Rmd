...
arima(Dict1["Mort2b.bc1(1.12)"], order=c(0,0,0), method="ML")
...
original = bxcx(Dict.res["Mort2b.bc1(1.12)"], InverseQ = TRUE, lambda = lambda)
...
```{r}
vect1
((Dict1["Mort1b(0)"])^lambda)[1:46] == Dict1["Mort2b.bc(0)"]

for(i in 1:46){
  data[i,2] = Mort1b$Mort1b[i]**lambda}
  data=data.frame(matrix(ncol=2, nrow=46))
  data[, 1] = Dict1["Mort2b.bc(0)"]

comp = data.frame(Dict1["Mort1b.bc"])
comp = data.frame("Mort1b" = Mort1b$Mort1b)
comp["Mort2b"] = NA
comp$Mort2b[1:46] = Mort2b$Mort2b[1:47]
comp$Mort2b[47:175] = forecast$pred
comp["Mort2b"][47:175] = NA
comp$Mort2b[47:175] = NA
comp = comp[,1:2]
Mort.pred = rbind(Mort2b, data.frame("Mort2b" = forecast$pred))
Mort.pred = 
forecast <- sarima.for(original, d=1, D=0, S = 12, p=0, q=0, Q=0, P=0, n.ahead=128)
arima()
predict(a, n.ahead = 128)
a = arima(Dict1["Mort2b.bc(0)"], order = c(0,1,0), seasonal = list(order=c(0,1,0), period=12), method="ML", include.mean=FALSE)
Dict1
a$residuals = ts(a$residuals)
...
original = bxcx(Dict1["Mort2b.bc(0)"], InverseQ = TRUE, lambda = lambda)
