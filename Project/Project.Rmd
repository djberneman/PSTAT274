...
# Mortality: [January 12, 2020 - May 24, 2023]
  # Daily
Mortality1a = ts(covid$Mortality[10:1238])
Mortality1a= data.frame(Mortality1a)
  # Weekly
Mortality1b=vector()
for(i in 1:175){
  x = 7 * (i-1) + 1
  y = 7 * i
  Mortality1b <- append(Mortality1b, mean(Mortality1a[x:y, 1]))
  Mortality1b = ts(Mortality1b)}
Mortality1b = data.frame(Mortality1b)
...
# Mortality: [January 12, 2020 - December 3, 2020]
  # Daily
Mortality2a = ts(covid$Mortality[10:336])
Mortality2a= data.frame(Mortality2a)
  # Weekly
Mortality2b=vector()
for(i in 1:46){
  x = 7 * (i-1) + 1
  y = 7 * i
  Mortality2b <- append(Mortality2b, mean(Mortality2a[x:y, 1]))
  Mortality2b = ts(Mortality2b)}
Mortality2b = data.frame(Mortality2b)
...
# Mortality: [December 4, 2020 - May 24, 2023]
  # Daily
Mortality3a = ts(covid$Mortality[337:1238])
Mortality3a= data.frame(Mortality3a)
  # Weekly
Mortality3b=vector()
for(i in 1:128){
  x = 7 * (i-1) + 1
  y = 7 * i
  Mortality3b <- append(Mortality3b, mean(Mortality3a[x:y, 1]))
  Mortality3b = ts(Mortality3b)}
Mortality3b = data.frame(Mortality3b)


# Vaccinated: [December 4, 2020 - May 24, 2023]
  # Daily
Vaccinated3a = ts(covid$people_vaccinated[337:1238])
Vaccinated3a= data.frame(Vaccinated3a)
  # Weekly
Vaccinated3b=vector()
for(i in 1:128){
  x = 7 * (i-1) + 1
  y = 7 * i
  Vaccinated3b <- append(Vaccinated3b, mean(Vaccinated3a[x:y, 1]))
  Vaccinated3b = ts(Vaccinated3b)}
Vaccinated3b = data.frame(Vaccinated3b)
...
vect = append(vect, Mortality1a)
vect = append(vect, Mortality1b)
vect = append(vect, Cases2a)
vect = append(vect, Cases2b)
vect = append(vect, Deaths2a)
vect = append(vect, Deaths2b)
vect = append(vect, Mortality2a)
vect = append(vect, Mortality2b)
vect = append(vect, Cases3a)
vect = append(vect, Cases3b)
vect = append(vect, Deaths3a)
vect = append(vect, Deaths3b)
vect = append(vect, Mortality3a)
vect = append(vect, Mortality3b)
vect = append(vect, Vaccinated3a)
vect = append(vect, Vaccinated3b)
...
vect1 = append(vect1, "Mortality1a")
vect1 = append(vect1, "Mortality1b")
vect1 = append(vect1, "Cases2a")
vect1 = append(vect1, "Cases2b")
vect1 = append(vect1, "Deaths2a")
vect1 = append(vect1, "Deaths2b")
vect1 = append(vect1, "Mortality2a")
vect1 = append(vect1, "Mortality2b")
vect1 = append(vect1, "Cases3a")
vect1 = append(vect1, "Cases3b")
vect1 = append(vect1, "Deaths3a")
vect1 = append(vect1, "Deaths3b")
vect1 = append(vect1, "Mortality3a")
vect1 = append(vect1, "Mortality3b")
vect1 = append(vect1, "Vaccinated3a")
vect1 = append(vect1, "Vaccinated3b")
```
\
...
log = list()
sqrt = list()
Data <- data.frame(matrix(ncol=19, nrow=480))
...
Data_AD_Pass = subset(Data, Data$`Anderson-Darling Test` > 0.05)[, c(1:12, 18, 20)]
Data_AD_Pass
Data_Shapiro_Pass = subset(Data, Data$`Shapiro test` > 0.05)[, c(1:12, 18, 20)]
Data_Shapiro_Pass
Data_ADF_Pass = subset(Data, Data$`Augmented Dickey-Fuller Test` < 0.05)[, c(1:12, 18, 20)]
Data_ADF_Pass
Data_KPSS_Pass = subset(Data, Data$`KPSS Test` >= 0.05)[, c(1:12, 18, 20)]
Data_KPSS_Pass
Data_Box_Pierce_Pass = subset(Data, Data$`Box-Pierce Test` > 0.05)[, c(1:12, 18, 20)]
Data_Box_Pierce_Pass
Data_Ljung_Box_Pass = subset(Data, Data$`Ljung-Box Test` > 0.05)[, c(1:12, 18, 20)]
Data_Ljung_Box_Pass
Data_McLeod_Li_Pass = subset(Data, Data$`McLeod-Li Test` > 0.05)[, c(1:12, 18, 20)]
Data_McLeod_Li_Pass

Data1 = subset(Data, Data$Pass >= 4)
Tier1 = subset(Data, Data$Pass >= 6)
Tier2 = subset(Data, Data$Pass >= 5 & Data$Pass < 6)
Tier3 = subset(Data, Data$Pass >= 4 & Data$Pass < 5)
```
...
for(i in 1:length(Mortality1a$Mortality1a)){
  Mortality1a$Mortality1a[i] == Mortality1aa[i+1] 
}
Mortality1aa == Mortality1a$Mortality1a - diff(Mortality1a$Mortality1a, 12)
Mortality1aa = diff(Mortality1a$Mortality1a, 12)


data = data.frame(matrix(ncol=6, nrow = length(Mortality1a$Mortality1a)))

arima(Mortality1a$Mortality1a, order = c(0,0,0), seasonal = list(order(c(0,1,0), period=7)), method = "ML")$residuals == diff(Mortality1a$Mortality1a, lag = 7)


nsdiffs(Mortality1b[,1])
nsdiffs(ts(Mortality1b$Mortality1b), test="ch")
Best_Models[[3]]$residuals == -Mortality1aa + Mortality1a$Mortality1a

for(t in 1:length(Best_Models)){
  op <- par("mfrow", "mar")
  par(mfrow = c(3, 2), mar = c(rep.int(2, 3), 0.5))
  res = Best_Models[[t]]$residuals
  name = paste0(Best_Models[[t]]$j, "(", Best_Models[[t]]$Name, ")")
  i = Best_Models[[t]]$i
  j = Best_Models[[t]]$j
  if(j %in% bc){
  ...
  periodogram(res, log = 'yes', main=paste0(name, " Periodogram"))
  periodogram(res, log = 'no', main=paste0(name, " Periodogram"))
  abline(h=0)
  ...
  qqnorm(res,main= paste0(name, " Q-Q Plot"))
  qqline(res,col="blue")
  x=acf2(res, main=name)
  mtext(paste0("Shapiro: p-value = ", signif(shapiro.test(res)$p.value, digits=3), "     McLeod-Li: p-value = ", signif(Box.test((res)^2, lag=sqrt(length(res)), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3), "     ad.test: p-value = ", signif(ad.test(res)$p.value, digits=3)), side=3, line=7, adj=1.15, cex=0.8)
  mtext(paste0("Box-Pierce: p-value = ", signif(Box.test(res, lag = sqrt(length(res)), type = c("Box-Pierce"), fitdf = 0)$p.value, digits=3), "     adf.test: p-value = ", signif(adf.test(res)$p.value, digits=3), "     Ljung-Box: p-value = ", signif(Box.test(res, lag=sqrt(length(res)), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3), "     kpss.test: p-value = ", signif(kpss.test(res)$p.value, digits=3)), side=3, line=6, adj=1.2, cex=0.8)
...
for(i in 1:length(vect)){
  ...
  for(j in 1:length(transform)){
    ...
    if(list[k]==0){
      ...
      mtext(paste0("Shapiro: p-value = ", signif(shapiro.test(difference[[k]])$p.value, digits=3), "     McLeod-Li: p-value = ", signif(Box.test((difference[[k]])^2, lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3), "     ad.test: p-value = ", signif(ad.test(difference[[k]])$p.value, digits=3)), side=3, line=7, adj=1.15, cex=0.8)
      mtext(paste0("Box-Pierce: p-value = ", signif(Box.test(difference[[k]], lag = sqrt(length(difference[[k]])), type = c("Box-Pierce"), fitdf = 0)$p.value, digits=3),"     Ljung-Box: p-value = ", signif(Box.test(difference[[k]], lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3), "     kpss.test: p-value = ", signif(kpss.test(difference[[k]])$p.value, digits=3)), side=3, line=6, adj=1.2, cex=0.8)}
    else{
      x=acf2(difference[[k]], main=diff_name[k+1])
      mtext(paste0("Shapiro: p-value = ", signif(shapiro.test(difference[[k]])$p.value, digits=3), "     McLeod-Li: p-value = ", signif(Box.test((difference[[k]])^2, lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3), "     ad.test: p-value = ", signif(ad.test(difference[[k]])$p.value, digits=3)), side=3, line=7, adj=1.15, cex=0.8)
      mtext(paste0("Box-Pierce: p-value = ", signif(Box.test(difference[[k]], lag = sqrt(length(difference[[k]])), type = c("Box-Pierce"), fitdf = 0)$p.value, digits=3),"     Ljung-Box: p-value = ", signif(Box.test(difference[[k]], lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3), "     kpss.test: p-value = ", signif(kpss.test(difference[[k]])$p.value, digits=3)), side=3, line=6, adj=1.2, cex=0.8)}}}}
...
x = arima(`Cases1a`, order = c(2,0,2), seasonal=list(order=c(3,1,4), period = 2), method="ML", include.mean = TRUE)

x = ARIMA_Cases1a_x$arma
y=acf2(x$residuals, main = x$call)
      mtext(paste0("Shapiro: p-value = ", signif(shapiro.test(x$residuals)$p.value, digits=3), "     McLeod-Li: p-value = ", signif(Box.test(x$residuals^2, lag=sqrt(length(x$residuals)), type=c("Ljung-Box"), fitdf = 2)$p.value, digits=3), "     ad.test: p-value = ", signif(ad.test(x$residuals)$p.value, digits=3)), side=1, line=-5.5, adj=1.15, cex=0.8)
      mtext(paste0("Box-Pierce: p-value = ", signif(Box.test(x$residuals, lag = sqrt(length(x$residuals)), type = c("Box-Pierce"), fitdf = 0)$p.value, digits=3),"     Ljung-Box: p-value = ", signif(Box.test(x$residuals, lag=sqrt(length(x$residuals)), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3), "     kpss.test: p-value = ", signif(kpss.test(x$residuals)$p.value, digits=3)), side=1, line=-6.5, adj=1.2, cex=0.8)


title = paste0(x$series, "Arima(", x$arma[1], ",", x$arma[6], ",", x$arma[2], ")x(", x$arma[3], ",", x$arma[7], ",", x$arma[4], ")_", x$arma[5])
x$arma[3]
          ...
          for(q in loop.q){
            for(p in loop.p){
              if((p==3 & d==1 & q==2 & P==1 & Q==1 & D==0) | (p==5 & d==1 & q==5 & P==1 & Q==1 & D==0) | (p==3 & d==1 & q==2 & P==2 & Q==1 & D==0) | (p==1 & d==1 & q==3 & P==2 & Q==1 & D==0) | (p==4 & d==1 & q==4 & P==2 & Q==1 & D==0)){
                Data[i, 1] = paste0("SARIMA(", p, ",", d, ",", q, ")x(", P, ",", D, ",", Q, ")_", s)
                assign(paste0("SARIMA(", p, ",", d, ",", q, ")x(", P, ",", D, ",", Q, ")_", s), 
                       arima(Mortality1a, order=c(p,d,q), seasonal=list(order=c(P,D,Q), period=s)))
                Data[i, 2] = AIC(get(paste0("SARIMA(", p, ",", d, ",", q, ")x(", P, ",", D, ",", Q, ")_", s)))
                x=acf2(get(paste0("SARIMA(", p, ",", d, ",", q, ")x(", P, ",", D, ",", Q, ")_", s))$residuals, 
                       main = paste0("SARIMA(", p, ",", d, ",", q, ")x(", P, ",", D, ",", Q, ")_", s, "  AICc = ", Data[i,2]))}
              else{
