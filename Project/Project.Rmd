...
Mortality = ts(covid$Mortality[10:1238])
Mortality = ts(covid$Mortality[1:336])
Mortality = ts(covid$Mortality[337:1238])
...
Data = data.frame(matrix(nrow=288, ncol=2))
      ...
      for(P in loop.P){
        for(Q in loop.Q){
          if(s == 0){
            ...
            Data[i, 2] = AIC(get(paste0("ARIMA(", p, "_1_", q, ")")))
            x=acf2(get(paste0("ARIMA(", p, "_1_", q, ")"))$residuals, main = paste0("ARIMA(", p, ",1,", q, ")   AICc = ", Data[i,2]))}
          else{
            Data[i, 1] = paste0("SARIMA(", p, "_1_", q, ")x((", P, ",", "1,", Q, ")_", s)
            assign(paste0("SARIMA(", p, "_1_", q, ")x((", P, ",", "1,", Q, ")_", s), arima(Mortality_1, order=c(p,0,q), seasonal=list(order=c(0,1,0), period=s), method="ML"))
            Data[i, 2] = AIC(get(paste0("SARIMA(", p, "_1_", q, ")x((", P, ",", "1,", Q, ")_", s)))
            x=acf2(get(paste0("SARIMA(", p, "_1_", q, ")x((", P, ",", "1,", Q, ")_", s))$residuals, main = paste0("SARIMA(", p, "_1_", q, ")x((", P, ",", "1,", Q, ")_", s, "  AICc = ", Data[i,2]))}
          i = i + 1}}}}}
...
```{r}
  ...
  x=arima(Mortality, order=c(0,1,0), seasonal = list(order = c(0,0,0), period = 0), method="ML")
  acf2(x
# Model
