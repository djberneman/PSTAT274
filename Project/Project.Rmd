...
log = list()
sqrt = list()
Data_a <- data.frame(matrix(ncol=20, nrow=480))
...
log = list()
sqrt = list()
Data <- data.frame(matrix(ncol=20, nrow=480))
...
for(x in 1:length(Best_Models_a)){
  ...
  if(min >= 4){
    loop.p = c(0,1,2,min-1,min)
    loop.q = c(0,1,2,min-1,min)}
  else{
    loop.p = c(seq(0,5))
    loop.q = c(seq(0,5))}
