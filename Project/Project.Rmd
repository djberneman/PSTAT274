...
data1 = data.frame(matrix(nrow=240, ncol=6))
colnames(data1) <- c("adf.test", "shapiro.test", "kpss.test", "Box-Pierce", "Ljung-Box", "McLeod-Li")
i=1
...
for(i in 1:length(vect)){
  ...
  for(j in 1:length(transform)){
    difference = vector()
    diff_name = vector()
    assign(paste0(transform_name[j], "adf.test"), list())
    assign(paste0(transform_name[j], "shapiro.test"), list())
    assign(paste0(transform_name[j], "kpss.test"), list())
    assign(paste0(transform_name[j], "Box-Pierce.test"), list())
    assign(paste0(transform_name[j], "Ljung-Box.test"), list())
    assign(paste0(transform_name[j], "McLeod-Li.test"), list())
    for(k in 1:length(list)){
      if(list[k]==0){
        diff_name = append(diff_name, transform_name[j])
        difference = append(difference, transform[j])}
    ...
    if(list[k]==0){
      op <- par("mfrow", "mar")
      par(mfrow = c(2, 2), mar = c(rep.int(3, 3), 0.5))
      paste0(transform_name[j], "adf.test") = append(paste0(transform_name[j], "adf.test"), ad.test(difference[[k]]))
      paste0(transform_name[j], "shapiro.test") = append(paste0(transform_name[j], "shapiro.test"), shapiro.test(difference[[k]]))
      paste0(transform_name[j], "kpss.test") = append(paste0(transform_name[j], "kpss.test"), kpss.test(difference[[k]]))
      paste0(transform_name[j], "Box-Pierce.test") = append(paste0(transform_name[j], "Box-Pierce.test"), Box.test(difference[[k]], lag = sqrt(length(difference[[k]])), type = c("Box-Pierce"), fitdf = 0))
      paste0(transform_name[j], "Ljung-Box.test") = append(paste0(transform_name[j], "Ljung-Box.test"), Box.test(difference[[k]], lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0))
      paste0(transform_name[j], "McLeod-Li.test") = append(paste0(transform_name[j], "Mc-Leod-Li.test"), Box.test((difference[[k]])^2, lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0))
      par(mfrow = op$mfrow, mar = op$mar)}
    else{
      op <- par("mfrow", "mar")
      par(mfrow = c(2, 2), mar = c(rep.int(3, 3), 0.5))
      ...
      paste0(transform_name[j], "Ljung-Box.test") = append(paste0(transform_name[j], "Ljung-Box.test"), Box.test(difference[[k]], lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0))
      paste0(transform_name[j], "McLeod-Li.test") = append(paste0(transform_name[j], "Mc-Leod-Li.test"), Box.test((difference[[k]])^2, lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0))}
    assign(paste0("tests_", transform_name[j]), list(get(paste0(transform_name[j], "adf.test")), get(paste0(transform_name[j], "shapiro.test")), get(paste0(transform_name[j], "kpss.test")), get(paste0(transform_name[j], "Box-Pierce.test")), get(paste0(transform_name[j], "Ljung-Box.test")), get(paste0(transform_name[j], "McLeod-Li.test"))))}}}
```
