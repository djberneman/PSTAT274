...
log = list()
sqrt = list()
Data_a <- data.frame(matrix(ncol=24, nrow=2112))
  ...
  for(j in 1:length(transform)){
    ...
    for(k in 1:length(list)){
      ...
      Variance_Order$Levels = c("Original", "Transformation", "Differencing", 
                                "Seasonality", "Current")
      Variance_Order$Variance[1:5] = Data_a[x, c(14:18)]
      ...
      Fails = 0
      if(Data_a[x, 14] < Data_a[x, 15]){
        Fails=Fails+1}
      if((Data_a[x, 15] < Data_a[x, 16]) & (Data_a[x, 15] < Data_a[x, 17])){
        Fails=Fails+1}
      if((Data_a[x, 14] < Data_a[x, 16]) & (Data_a[x, 14] < Data_a[x, 17])){
        Fails=Fails+1}
      if((Data_a[x, 16] < Data_a[x, 18]) & (Data_a[x, 17] < Data_a[x, 18])){
        Fails=Fails+1}
      if(Data_a[x, 14] < Data_a[x, 18]){
        Fails=Fails+1}
      if(Data_a[x, 15] < Data_a[x, 18]){
        Fails=Fails+1}
...
Stat_Norm_Pass_a = subset(Stat_Norm_a, Stat_Norm_a$Pass >=4)
Stat_Res_a = subset(Stationarity_a, Stationarity_a$Residual_Tests != "Fail")
Stat_Res_Pass_a = subset(Stat_Res_a, Stat_Res_a$Pass >=4)
...
plot_fxn = function(name, res){
  ...
  abline(h=mean(res), col= "blue")
  abline(lm(res ~ as.numeric(1:length(res))), col="red")
  TSA::TSA::periodogram(Dict.res["Vax3b.log(12)"], log = 'yes', main=paste0(name, " Periodogram"), cex.main=0.6)
  TSA::periodogram(res, log = 'no', main=paste0(name, " Periodogram"), cex.main=0.6)
  abline(h=0)
  hist(res, col="light blue", xlab="", main=name, prob=TRUE)
  mtext(paste0("Variance = ", round(var(res), digits = 4)), side=1, line = 3, cex = 0.8)
...
forecast <- sarima.for(original, n.ahead=1)

original = bxcx(`Mort2b.bc(1.12)`, InverseQ = TRUE, lambda = lambda)
