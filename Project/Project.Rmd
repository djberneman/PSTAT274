...
log = list()
sqrt = list()
Data <- data.frame(matrix(ncol=12, nrow=480))
colnames(Data) <- c("Name", "Dataset", "Average", "Transformation", "Differencing", 
                    "Anderson-Darling Test", "Shapiro test", "Augmented Dickey-Fuller Test", 
                    "KPSS Test", "Box-Pierce Test", "Ljung-Box Test", "McLeod-Li Test")
...
# Data %>% 
# mutate(
#   `Anderson-Darling Test` = if_else(
#     `Anderson-Darling Test` < 0.01, 
#     formatC(`Anderson-Darling Test`, digits = 2, format = "e"), 
#     formatC(`Anderson-Darling Test`, digits = 2, format = "f", drop0trailing = FALSE))) %>% 
# mutate_if(
#   is.numeric, 
#   function(x){
...
Data_AD_Pass = subset(Data, Data$`Anderson-Darling Test` > 0.05)[, c(1:6, 13)]
Data_AD_Pass
Data_Shapiro_Pass = subset(Data, Data$`Shapiro test` > 0.05)[, c(1:5, 7, 13)]
Data_Shapiro_Pass
Data_ADF_Pass = subset(Data, Data$`Augmented Dickey-Fuller Test` < 0.05)[, c(1:5, 8,13)]
Data_ADF_Pass
Data_KPSS_Pass = subset(Data, Data$`KPSS Test` >= 0.05)[, c(1:5, 9,13)]
Data_KPSS_Pass
Data_Box_Pierce_Pass = subset(Data, Data$`Box-Pierce Test` > 0.05)[, c(1:5, 10,13)]
Data_Box_Pierce_Pass
Data_Ljung_Box_Pass = subset(Data, Data$`Ljung-Box Test` > 0.05)[, c(1:5, 11,13)]
Data_Ljung_Box_Pass
Data_McLeod_Li_Pass = subset(Data, Data$`McLeod-Li Test` > 0.05)[, c(1:5, 12,13)]
Data_McLeod_Li_Pass
...
Tier1
Tier2 = subset(Data, Data$Pass == 4)
Tier2
Tier3 = subset(Data, Data$Pass == 3)
Tier3
```
...
for(i in 1:length(vect)){
  ...
  for(j in 1:length(transform)){
    ...
    if(list[k]==0){
      ...
      mtext(paste0("Shapiro: p-value = ", signif(shapiro.test(difference[[k]])$p.value, digits=3), "     McLeod-Li: p-value = ", signif(Box.test((difference[[k]])^2, lag=sqrt(length(difference[[k]])), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3), "     ad.test: p-value = ", signif(ad.test(difference[[k]])$p.value, digits=3)), side=3, line=2.5, adj=1.15, cex=0.8)
