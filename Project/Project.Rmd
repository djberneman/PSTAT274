...
for(i in 1:length(vect)){
  ...
  for(j in 1:length(transform)){
    ...
    for(k in 1:length(list)){
      ...
      if(list[k]==1){
        assign(paste0(transform_name[j], "(", list[k], ")"),  arima(transform[[j]], order = c(0,1,0), method="ML"))
        difference = append(difference, get(paste0(transform_name[j], "(", list[k], ")")))}
      if(list[k]==7 | list[k]==12){
        assign(paste0(transform_name[j], "(", list[k], ")"),  arima(transform[[j]], order = c(0,0,0), seasonal=list(order=c(0,1,0), period = list[k]), method="ML"))
        difference = append(difference, get(paste0(transform_name[j], "(", list[k], ")")))}
      if(list[k]==1.7){
        assign(paste0(transform_name[j], "(", list[k], ")"),  arima(transform[[j]], order = c(0,1,0), seasonal=list(order=c(0,1,0), period = 7), method="ML"))
        difference = append(difference, get(paste0(transform_name[j], "(", list[k], ")")))}
      if(list[k]==1.12){
        assign(paste0(transform_name[j], "(", list[k], ")"),  arima(transform[[j]], order = c(0,1,0), seasonal=list(order=c(0,1,0), period = 12), method="ML"))
        difference = append(difference, get(paste0(transform_name[j], "(", list[k], ")")))}}
    ...
    for(x in 1:length(difference)){
      if(x==0){
        ...
        model=ets(difference[[x]])
        print(summary(model))
        x=acf2(difference[[x]], main=diff_name[x])
        mtext(paste0("Shapiro: p-value = ", signif(shapiro.test(difference[[x]])$p.value, digits=3), "     McLeod-Li: p-value = ", signif(Box.test((difference[[x]])^2, lag=sqrt(length(difference[[x]])), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3), "     ad.test: p-value = ", signif(ad.test(difference[[x]])$p.value, digits=3)), side=3, line=2.5, adj=1.375, cex=0.8)
        mtext(paste0("Box-Pierce: p-value = ", signif(Box.test(difference[[x]], lag = sqrt(length(difference[[x]])), type = c("Box-Pierce"), fitdf = 0)$p.value, digits=3),"     Ljung-Box: p-value = ", signif(Box.test(difference[[x]], lag=sqrt(length(difference[[x]])), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3), "     kpss.test: p-value = ", signif(kpss.test(difference[[x]])$p.value, digits=3)), side=3, line=1.5, adj=1.5, cex=0.8)
        require(TSA)
        # nf <- par(mfrow=c(2,2))
        plot.ts(difference[[x]], main=diff_name[x])
        abline(h=mean(difference[[x]]), col= "blue")
        abline(lm(difference[[x]] ~ as.numeric(1:length(difference[[x]]))), col="red")
        periodogram(difference[[x]], log = 'yes', main=paste0(diff_name[x], " Periodogram"))
        abline(h=0)
        hist(difference[[x]], col="light blue", xlab="", main=diff_name[x], prob=TRUE)
        mtext(paste0("Variance = ", var(difference[[x]])), side=1, line = 2)
        m <- mean(difference[[x]])
        std <- sqrt(var(difference[[x]]))
        curve( dnorm(x,m,std), add=TRUE)
        qqnorm(difference[[x]],main= paste0(diff_name[x], " Q-Q Plot"))
        qqline(difference[[x]],col="blue")
        par(mfrow = op$mfrow, mar = op$mar)}
      else{
        op <- par("mfrow", "mar")
        par(mfrow = c(2, 2), mar = c(rep.int(3, 3), 0.5))
        model=ets(difference[x]$residuals)
        print(summary(model))
        x=acf2(difference[x]$residuals, main=diff_name[x])
        mtext(paste0("Shapiro: p-value = ", signif(shapiro.test(difference[x]$residuals)$p.value, digits=3), "     McLeod-Li: p-value = ", signif(Box.test((difference[x]$residuals)^2, lag=sqrt(length(difference[x]$residuals)), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3), "     ad.test: p-value = ", signif(ad.test(difference[x]$residuals)$p.value, digits=3)), side=3, line=2.5, adj=1.375, cex=0.8)
        mtext(paste0("Box-Pierce: p-value = ", signif(Box.test(difference[x]$residuals, lag = sqrt(length(difference[x]$residuals)), type = c("Box-Pierce"), fitdf = 0)$p.value, digits=3),"     Ljung-Box: p-value = ", signif(Box.test(difference[x]$residuals, lag=sqrt(length(difference[x]$residuals)), type=c("Ljung-Box"), fitdf = 0)$p.value, digits=3), "     kpss.test: p-value = ", signif(kpss.test(difference[x]$residuals)$p.value, digits=3)), side=3, line=1.5, adj=1.5, cex=0.8)
        require(TSA)
        # nf <- par(mfrow=c(2,2))
        plot.ts(difference[x]$residuals, main=diff_name[x])
        abline(h=mean(difference[x]$residuals), col= "blue")
        abline(lm(difference[x]$residuals ~ as.numeric(1:length(difference[x]$residuals))), col="red")
        periodogram(difference[x]$residuals, log = 'yes', main=paste0(diff_name[x], " Periodogram"))
        abline(h=0)
        hist(difference[x]$residuals, col="light blue", xlab="", main=diff_name[x], prob=TRUE)
        mtext(paste0("Variance = ", var(difference[x]$residuals)), side=1, line = 2)
        m <- mean(difference[x]$residuals)
        std <- sqrt(var(difference[x]$residuals))
        curve( dnorm(x,m,std), add=TRUE)
        qqnorm(difference[x]$residuals,main= paste0(diff_name[x], " Q-Q Plot"))
        qqline(difference[x]$residuals,col="blue")
        par(mfrow = op$mfrow, mar = op$mar)}}
