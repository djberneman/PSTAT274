...
# This bring the total number of models being evaluated to 480.
  ...
  for(j in 1:length(transform)){
    ...
    for(k in 1:length(list)){
      ...
      if(Data$Pass[x] >= 4){
        if(list[k]==0){
          assign(paste0(transform_name[j], "(0,0,0)"), transform[[j]])
          `Best_Models(0,0,0)` = append(`Best_Models(0,0,0)`, list(get(paste0(transform_name[j], "(0,0,0)"))))}
        if(list[k]==1){
          assign(paste0(transform_name[j], "(0,1,0)"), arima(
            transform[[j]], order = c(0,1,0), method="ML"))
          Best_Models = append(Best_Models, list(get(paste0(transform_name[j], "(0,1,0)"))))}
        if(list[k]==7){
          assign(paste0(transform_name[j], "(0,7,0)"), arima(
            transform[[j]], order = c(0,7,0), method="ML"))
          Best_Models = append(Best_Models, list(get(paste0(transform_name[j], "(0,7,0)"))))}
        if(list[k]==12){
          assign(paste0(transform_name[j], "(0,12,0)"), arima(
            transform[[j]], order = c(0,12,0), method="ML"))
          Best_Models = append(Best_Models, list(get(paste0(transform_name[j], "(0,12,0)"))))}
        if(list[k]==1.7){
          assign(paste0(transform_name[j], "(0,1,0)x(0,1,0)_7"), arima(
            transform[[j]], order = c(0,1,0), seasonal=list(
            order=c(0,1,0), period = 7), method="ML"))
          Best_Models = append(Best_Models, list(get(
            paste0(transform_name[j], "(0,1,0)x(0,1,0)_7"))))}
        if(list[k]==1.12){
          assign(paste0(transform_name[j], "(0,1,0)x(0,1,0)_12"), arima(
            transform[[j]], order = c(0,1,0), seasonal=list(
            order=c(0,1,0), period = 12), method="ML"))
          Best_Models = append(Best_Models, list(get(
            paste0(transform_name[j], "(0,1,0)x(0,1,0)_12"))))}}
      x = x + 1}}}
...
Data1 = subset(Data, Data$Pass >= 3)
Data1
Tier1 = subset(Data, Data$Pass >= 5)
Tier1
Tier2 = subset(Data, Data$Pass >= 4 & Data$Pass < 5)
Tier2
Tier3 = subset(Data, Data$Pass >= 3 & Data$Pass < 4)
Tier3
```
...
for(i in 1:length(vect)){
  ...
  for(j in 1:length(transform)){
    ...
    if(list[k]==0){
      ...
      abline(h=0)
      periodogram(difference[[k]], log = 'no', main=paste0(diff_name[k+1], " Periodogram"))
      abline(h=0)
