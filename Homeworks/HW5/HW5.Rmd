...
covid_world <- covid_world[12:1225,]
...
for(i in 1:1214){
  if(i >= 365){
    covid_world$`365Day_Rolling_Fatality`[i] = mean(covid_world$Smoothed_New_Case_Fatality_Rate[(i-364):i], na.rm = TRUE)}
  else{
    covid_world$`365Day_Rolling_Fatality`[i] = NA
  }}

Mortality = ts(covid_world$Smoothed_New_Case_Fatality_Rate)

plot.ts(Mortality)

# The next 3 functions do not work unless my frequency is 2 or more
# plot(stl(Mortality, s.window="periodic"))
# plot(decompose(Mortality, type="multiplicative"))
# plot(decompose(Mortality, type="additive"))

Rolling_Mortality = ts(covid_world$`365Day_Rolling_Fatality`)[365:1214]

plot.ts(Rolling_Mortality)

# The next 3 functions do not work unless my frequency is 2 or more
# plot(stl(Rolling_Mortality, s.window="periodic"))
# plot(decompose(Rolling_Mortality, type="multiplicative"))
# plot(decompose(Rolling_Mortality, type="additive"))

# The following series is an alternative time series for the project

Deaths = ts(covid_world$new_deaths)

plot.ts(Deaths)

# The next 3 functions do not work unless my frequency is 2 or more
# plot(stl(Deaths, s.window="periodic"))
# plot(decompose(Deaths, type="multiplicative"))
# plot(decompose(Deaths, type="additive"))
```
\
...
# Mortality models
# Model 1
Mortality_model1 <- auto.arima(Mortality, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=FALSE, seasonal = TRUE, trace = TRUE)
...
# Model 2
Mortality_model2 <- auto.arima(Mortality, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=TRUE, seasonal = TRUE, trace = TRUE)
