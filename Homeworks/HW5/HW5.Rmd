...
```{r}
x = auto.arima(milk,trace = TRUE, D=1, start.p = 0, start.q = 0, start.P = 0, start.Q = 0)
...
plot(sarima(milk, 0,1,0,0,1,0,12))

points(x$fitted, col="red")
...
x = ARIMA(milk, order = c(2,1,0), seasonal=list(order=c(0,1,0), period=12), xreg = list(ar = c(0.4, 0.5), sar = c(0.7)))
InvertibleQ(c(0.8, -0.5))
polyroot(c(-0.5, 0.1, 1))
plot(sarima(milk, 0,1,0,0,1,0,12))
```
