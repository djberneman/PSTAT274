...
arima.sim(model, n, innov = rand.gen(n, ...), n.start = NA)\

  - *only useful to make a sample or theoretical model for pure ARIMA*
  ...
  - *`n.start` length of ‘burn-in’ period. If NA, the default, a reasonable value is computed. (set to 0 if `innov` is an existing time series*\
\
...
ARMAacf(ar = numeric(), ma = numeric(), lag.max = r, pacf = FALSE)\
  ...
  - *Use to calculate theoretical autocorrelations*
...
```{r}
# auto.arima
plot.ts(auto.arima(milk, trace = TRUE, D=1, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, seasonal = TRUE)$residuals)

# arima
arima = arima(milk, order=c(1,0,0), seasonal = list(order = c(1,1,1), period=12))
