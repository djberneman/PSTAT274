...
fit.ar1 = ar(Mortality.diff, aic = FALSE, method="mle")
fit.ar2 = ar(Mortality.diff, aic = FALSE, order.max=12, method="yw")
fit.ar3 = ar(Mortality.diff, aic = TRUE, method="mle")
fit.ar4 = ar(Mortality.diff, aic = TRUE, order.max = 12, method="yw")
...
fit.ar1 <- arima(Mortality.diff, order=c(12,1,0))
fit.ar2 <- arima(Mortality.diff, order=c(10,1,0))
fit.ar3 <- arima(Mortality.diff, order=c(12,0,0))
fit.ar4 <- arima(Mortality.diff, order=c(10,0,0))
...
fit.arma9 <- arima(Mortality.diff, order=c(12,1,9))
fit.arma10 <- arima(Mortality.diff, order=c(12,1,10))
fit.arma11 <- arima(Mortality.diff, order=c(12,1,11))
fit.arma12 <- arima(Mortality.diff, order=c(12,1,12))

fit.arma13 <- arima(Mortality.diff, order=c(12,0,9))
fit.arma14 <- arima(Mortality.diff, order=c(12,0,10))
fit.arma15 <- arima(Mortality.diff, order=c(12,0,11))
fit.arma16 <- arima(Mortality.diff, order=c(12,0,12))

fit.arma17 <- arima(Mortality.diff, order=c(10,1,9))
fit.arma18 <- arima(Mortality.diff, order=c(10,1,10))
fit.arma19 <- arima(Mortality.diff, order=c(10,1,11))
fit.arma20 <- arima(Mortality.diff, order=c(10,1,12))

fit.arma21 <- arima(Mortality.diff, order=c(10,0,9))
fit.arma22 <- arima(Mortality.diff, order=c(10,0,10))
fit.arma23 <- arima(Mortality.diff, order=c(10,0,11))
fit.arma24 <- arima(Mortality.diff, order=c(10,0,12))
...
acvf = acf(Mortality, plot=FALSE, lag.max = length(Mortality))$acf[,1,1] * var(Mortality) 
m = length(acvf)
...
fit.ma9 <- arima(Mortality.diff, order=c(0,0,9))
fit.ma10 <- arima(Mortality.diff, order=c(0,0,10))
fit.ma11 <- arima(Mortality.diff, order=c(0,0,11))
fit.ma12 <- arima(Mortality.diff, order=c(0,0,12))
...
# Mortality models
# Model 1
Mortality_model1 <- auto.arima(Mortality.diff2, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=FALSE, seasonal = TRUE, trace = TRUE)

Mortality_model1$coef

polyroot(c(1, Mortality_model1$coef[1], Mortality_model1$coef[2], Mortality_model1$coef[3], Mortality_model1$coef[4], Mortality_model1$coef[5]))

Mortality_acf1 = acf2(Mortality_model1$residuals, max.lag = 200)

plot.ts(Mortality, lwd=4)
points(Mortality_model1$fitted, type="l", col="red")

# Model 2
Mortality_model2 <- auto.arima(Mortality.diff2, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=TRUE, seasonal = TRUE, trace = TRUE)

Mortality_model2$coef

polyroot(c(1, Mortality_model2$coef[1], Mortality_model2$coef[2]))

Mortality_acf2 = acf2(Mortality_model2$residuals, max.lag = 200)

plot.ts(Mortality, lwd=4)
points(Mortality_model2$fitted, type="l", col="red")


# Rolling_Mortality Models
# Model 1
Rolling_Mortality_model1 <- auto.arima(Rolling_Mortality, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=FALSE, seasonal = TRUE, trace = TRUE)

Rolling_Mortality_model1$coef

polyroot(c(1, Rolling_Mortality_model1$coef[1], Rolling_Mortality_model1$coef[2], Rolling_Mortality_model1$coef[3], Rolling_Mortality_model1$coef[4], Rolling_Mortality_model1$coef[5]))

Rolling_Mortality_acf1 = acf2(Rolling_Mortality_model1$residuals, max.lag = 200)

plot.ts(Rolling_Mortality, lwd=4)
points(Rolling_Mortality_model1$fitted, type="l", col="red")

# Model 2
Rolling_Mortality_model2 <- auto.arima(Rolling_Mortality, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=TRUE, seasonal = TRUE, trace = TRUE)

Rolling_Mortality_model2$coef

Rolling_Mortality_acf2 = acf2(Rolling_Mortality_model2$residuals, max.lag = 200)

plot.ts(Rolling_Mortality, lwd=4)
points(Rolling_Mortality_model2$fitted, type="l", col="red")


# Deaths Models
# Model 1
Deaths_model1 <- auto.arima(Deaths, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=FALSE, seasonal = TRUE, trace = TRUE)

Deaths_model1$coef

polyroot(c(1, Deaths_model1$coef[1], Deaths_model1$coef[2], Deaths_model1$coef[3], Deaths_model1$coef[4], Deaths_model1$coef[5]))

Deaths_acf1 = acf2(Deaths_model1$residuals, max.lag =600)

plot.ts(Deaths, lwd=4)
points(Deaths_model1$fitted, type="l", col="red")

# Model 2
Deaths_model2 <- auto.arima(Deaths, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=TRUE, seasonal = TRUE, stepwise=FALSE, approximation=FALSE, trace = TRUE)

Deaths_model2$coef

# Closest to being stationary; can possibly be made stationary if some of the coefficients are given a slight change
polyroot(c(1, Deaths_model2$coef[1], Deaths_model2$coef[2], Deaths_model2$coef[3], Deaths_model2$coef[4]))

polyroot(c(1, Deaths_model2$coef[1]+.01, Deaths_model2$coef[2]-.005, Deaths_model2$coef[3]+.015, Deaths_model2$coef[4]-.023))

Deaths_acf2 = acf2(Deaths_model2$residuals, max.lag =600)

plot.ts(Deaths, lwd=3)
points(Deaths_model2$fitted, type="l", col="red")


# Model 3 (based on fixed model coefficients from model 2)

Deaths_model3 <- auto.arima(Deaths, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=FALSE, seasonal = TRUE, fixed=c(Deaths_model2$coef[1]+.01, Deaths_model2$coef[2]-.005, Deaths_model2$coef[3]+.015, Deaths_model2$coef[4]-.023, mu = Deaths_model2$coef[5]), stepwise=FALSE, approximation=FALSE, trace = TRUE)

Deaths_model3$coef

# This is stationary
polyroot(c(1, Deaths_model3$coef[1], Deaths_model3$coef[2], Deaths_model3$coef[3], Deaths_model3$coef[4]))

Deaths_acf3 = acf2(Deaths_model3$residuals, max.lag =600)

plot.ts(Deaths, lwd=3)
points(Deaths_model3$fitted, type="l", col="red")
...
Mortality_box <- auto.arima(Mortality.bc, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=TRUE, seasonal = TRUE, trace = TRUE)

# # This is a pure MA(5) model which is always stationary
Mortality_box$coef

Mortality_acf3 = acf2(Mortality_box$residuals, max.lag = 100)

#log transform
Mortality.log = log(Mortality)
Mortality_log <- auto.arima(Mortality.log, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=TRUE, seasonal = TRUE, trace = TRUE)

# # This is a pure MA(5) model which is always stationary
Mortality_log$coef

Mortality_acf4 = acf2(Mortality_log$residuals, max.lag=600)

# square root transform
Mortality.sqrt = sqrt(Mortality)
Mortality_sqrt <- auto.arima(Mortality.sqrt, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=TRUE, seasonal = TRUE, trace = TRUE)

# # This is a pure MA(5) model which is always stationary
Mortality_sqrt$coef

Mortality_acf5 = acf2(Mortality_sqrt$residuals, max.lag = 600)


# Rolling_Mortality

# Transform 
t = 1:length(Rolling_Mortality)
fit = lm(Rolling_Mortality ~ t)

# Box-Cox Transform
bcTransform = boxcox(Rolling_Mortality ~ t,plotit = TRUE)

lambda = bcTransform$x[which(bcTransform$y == max(bcTransform$y))] 
Rolling_Mortality.bc = (1/lambda)*(Rolling_Mortality^lambda-1)

Rolling_Mortality_box <- auto.arima(Rolling_Mortality.bc, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=TRUE, seasonal = TRUE, trace = TRUE)

Rolling_Mortality_box$coef

Rolling_Mortality_acf3 = acf2(Rolling_Mortality_box$residuals, max.lag = 800)

#log transform
Rolling_Mortality.log = log(Rolling_Mortality)
Rolling_Mortality_log <- auto.arima(Rolling_Mortality.log, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=TRUE, seasonal = TRUE, trace = TRUE)

Rolling_Mortality_log$coef

Rolling_Mortality_acf4 = acf2(Rolling_Mortality_log$residuals, max.lag=800)

# square root transform
Rolling_Mortality.sqrt = sqrt(Rolling_Mortality)
Rolling_Mortality_sqrt <- auto.arima(Rolling_Mortality.sqrt, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=TRUE, seasonal = TRUE, trace = TRUE)

Rolling_Mortality_sqrt$coef

acf2(Rolling_Mortality_sqrt$residuals, max.lag = 800)
...
Deaths_sqrt <- auto.arima(Deaths.sqrt, start.p = 0, start.q = 0, start.P = 0, start.Q = 0, stationary=TRUE, seasonal = TRUE, trace = TRUE)

Deaths_sqrt$coef

x = Deaths_sqrt$coef

polyroot(c(1, x[1], x[2], x[3]))

Deaths_acf4 = acf2(Deaths_sqrt$residuals, max.lag = 800)
```
```{r}
ts.plot(Mortality)
acf(Mortality) 
pacf(Mortality)
Mortality.diff <- diff(Mortality, 2)
ts.plot(Mortality.diff)
acf(Mortality.diff)
pacf(Mortality.diff)
auto.arima(Mortality.diff)

fit.ar =ar(Mortality.diff, method = c("yw")) 

ar(Mortality.diff, method = c("yw"))
ar(Mortality, aic = FALSE, order.max = NULL, method = c("mle"))
ar(Mortality, aic = TRUE, order.max = 5, method = c("yw"))
ar(Mortality, aic = FALSE, method = c("yw"))
arima(Mortality, order = c(2,1,3))
```


\
For both series, I attempted boxcox, square root, and log transformations. Some of them failed to work, while others would provide me with a series that when estimating the ARIMA order and coefficients, I would get an ARMA(0,0). For the ones the provided me with a legitimate ARIMA model, I used the polyroot function with the estimated coefficients to see if any transformed series was stationary, but none of them were.  
